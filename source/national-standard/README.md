# 学习游戏 Standards Core 依据包

生成日期：2026-06-08

这个包用于保存课程标准资料，作为游戏内容设定、关卡目标、任务标签、能力观察点和来源引用的依据。

## 先读哪个文件

1. `learning-game-standards-reference.md`：标准来源总览和游戏使用重点。
...
5. `available-standard-outcome-seeds.jsonl`：当前可用的 outcome/priority seeds，不代表全量标准抽取。

## 包内结构

- `original-files/source/national-standard/`：完整原始标准文件，保留原目录结构。
- `processed-and-indexes/`：现有 processed、resource card、standards integration 和审计资料。
...
- 如果未来要把某一条标准做成正式学习活动，仍需要单独走设计和发布审核。
